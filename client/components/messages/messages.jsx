Messages = React.createClass({
  ...
  getMeteorData() {
    ...
    data.messages = DBMessage.find({$or: [{'to._id': Meteor.userId()}, {'fromuser': Meteor.userId()}]}).fetch();
    return data;
  },
  removeMessage(e) {
    e.preventDefault();
  ...
  render() {
    var rows = this.data.messages.map(function(message) {
      ...
      if (message.fromuser === Meteor.userId()) {
        klass = 'primarty-font green text-muted';
      }
    ...
    return (
      <div className="column col-sm-7 col-xs-1 sidebar-offcanvas" id="sidebar">
        <ul className="nav">
          <li>
            <a href="#" className="visible-xs text-center"><i className="fa fa-list-alt"></i></a>
          </li>
        </ul>
